...
<div>
  ...
  <div  class="m-container-small m-padded-tb-big">
    ...
    <div class="ui middle aligned two column grid">
        ...
        共 <h2 class="ui orange header m-inline-block m-text-thin" v-text="tags.length"> 14 </h2> 个
        </div>
    </div>
    </div>

    <div class="ui attached segment m-padded-tb-large">
    <div class="ui labeled button m-margin-tb-tiny" v-for="(tag, iterStat) in tags" :key="iterStat">
        <a v-on:click="getTag(activeTagId = tag.id)" class="ui button"
            ...
            :style="{color:(tag.id === activeTagId ? 'yellow' : '')}"
            >
            {{tag.name}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{tag.blogs.length}}</a>
    </div>
    </div>
    ...
    <div class="ui top attached teal segment">
        ...
        <div class="ui middle aligned mobile reversed stackable grid">
            ...
            <div class="ui grid">
                ...
                <a @click="activeTagId=tag.id;getTag()" class="ui pink basic left pointing label m-padded-tiny" v-text="tag.name">认知升级</a>
                </div>
...
export default {
  ...
  methods: {
    pagination (page) {
      this.$http({
        method: 'POST',
        url: this.$base_url + '/x/tags?activeTagId=' + this.activeTagId,
        data: page
      }).then(response => {
    ...
    getTagsAll () {
      ...
      this.$http({
        method: 'GET',
        url: this.$base_url + '/x/tagsAll?activeTagId=' + this.activeTagId
      }).then(response => {
        this.blogs = response.data.data.blogs
        this.tags = response.data.data.tags
        this.activeTagId = response.data.data.activeTagId
        this.page = response.data.data.page
        this.loading = false
      })
    },
  ...
  created () {
    this.activeTagId = (this.$route.query.activeTagId === undefined ? this.activeTagId : this.$route.query.activeTagId)
    this.getTagsAll()
  }
}
</script>
