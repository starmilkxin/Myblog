...
public class TagController {
    ...
    public String input(@RequestBody Map<String, Object> receivedData) {
        ...
        if (tag_same != null) {
            return JSON.toJSONString(Result.error().setMessage("无法添加重复的标签").setCode(Result.ERROR_REPEAT));
        }
        int n = 0;
        //新增
        if (tag.getId() == -1) {
        ...
        if (n == 0) {
            return JSON.toJSONString(Result.error().setMessage("插入/更新tag失败"));
        }else {
    ...
    public String delete(@RequestParam("id") Integer id, @RequestParam("pageCurrent") int pageCurrent) {
        ...
        if (n == 0) {
            return JSON.toJSONString(Result.error().setMessage("删除tag失败"));
        }else {
            Page page = new Page();
